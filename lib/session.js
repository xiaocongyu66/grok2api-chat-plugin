...
export function clearHistory(e) {
  ...
  if (s && s.lastResponseId) {
    sessions.set(sk, { ...s, lastResponseId: "", lastResponseAt: 0 })
  }
  scheduleSave()
...
/** Responses API previous_response_id 粘滞（按群/私聊 scope） */
export function getLastResponseId(e) {
  ensureLoaded()
  const s = sessions.get(scopeKey(e))
  return s?.lastResponseId ? String(s.lastResponseId) : ""
}

export function setLastResponseId(e, responseId) {
  ensureLoaded()
  const k = scopeKey(e)
  const prev = sessions.get(k) || { active: false, scope: k }
  const id = String(responseId || "").trim()
  sessions.set(k, {
    ...prev,
    lastResponseId: id,
    lastResponseAt: id ? Date.now() : 0,
  })
  scheduleSave()
  return id
}
