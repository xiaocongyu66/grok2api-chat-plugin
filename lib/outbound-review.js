...
/**
 * 出站审查入口 — **群聊与私聊均生效**（不区分 isGroup）
 * @param {string} text
 * @param {object} [cfg]
 * @param {{ channel?: 'group'|'private'|'unknown' }} [meta]
 * @returns {{ forward: boolean, nsfw: boolean, method: string, score?: number, hits?: string[], reason?: string, channel?: string }}
 */
export async function reviewOutboundContent(text, cfg, meta = {}) {
  const c = cfg || Config.get()
  const channel = meta.channel || "unknown"

  if (c.outboundReviewEnable === false && c.chatNsfwForward === false) {
    return { forward: false, nsfw: false, method: "off", reason: "disabled", channel }
  }
  ...
  // 可选：仅群 / 仅私 / 全部（默认 all）
  const scope = String(c.outboundReviewScope || "all").toLowerCase()
  if (scope === "group" && channel === "private") {
    return { forward: false, nsfw: false, method: "skip-scope", reason: "scope-group-only", channel }
  }
  if (scope === "private" && channel === "group") {
    return { forward: false, nsfw: false, method: "skip-scope", reason: "scope-private-only", channel }
  }
  ...
  if (useAi) {
    try {
      ...
      logger?.warn?.(
        `[grok2api-chat-plugin] 出站 AI 审查失败(${channel})，回退关键词: ${e.message}`,
      )
    }
  }
