 ...
 */
async function postChatCompletions(messages, model, { tools, toolChoice } = {}) {
...
/**
 * 对话 + 媒体工具循环（generate_image / generate_video）
 * 结果供上层合并聊天记录转发
 */
export async function chatWithMediaTools({
  ...
  const mediaResults = []
  let last = null
  const rounds = Math.min(5, Math.max(1, Number(maxRounds) || 3))
  ...
  for (let i = 0; i < rounds; i++) {
    ...
    for (const tc of assistantMsg.tool_calls) {
      ...
      if (exec.media) mediaResults.push(exec.media)
      try {
  ...
  if (!content.trim() && mediaResults.length) {
    const imgs = mediaResults.filter(m => m.type === "image").length
    const vids = mediaResults.filter(m => m.type === "video").length
    const parts = []
    if (imgs) parts.push(`已生成 ${imgs} 组图片`)
    if (vids) parts.push(`已生成 ${vids} 个视频`)
    content = `${parts.join("，")}（见合并转发）`
  }
  if (!content.trim() && !mediaResults.length) {
    throw new Error(
      `对话返回为空（model=${last?.model}）。请检查账号池与模型是否支持 tools`,
    )
  }
